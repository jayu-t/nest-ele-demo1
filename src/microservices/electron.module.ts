...
import {
  ElectronModuleAsyncOptions,
  ElectronModuleOptions,
  ElectronModuleProviderValue,
} from './interface/electron-module-options.interface';
import {
  ELECTRON_MODULE_PROVIDER_VALUE,
  ELECTRON_WINDOW,
...
export class ElectronModule {
  ...
  static register(options: ElectronModuleOptions): DynamicModule {
    // !isElectron &&
    //   ElectronModule.logger.warn(
    //     'Not in Electron environment, all providers from ElectronModule will be null',
    //   );

    const provideName = `${ELECTRON_WINDOW}:${
      options.name || ELECTRON_WINDOW_DEFAULT_NAME
    }`;
    ...
    providers.push(
      {
        ...
        useFactory: (value: ElectronModuleProviderValue) =>
          value instanceof BrowserWindow ? value : value.win,
