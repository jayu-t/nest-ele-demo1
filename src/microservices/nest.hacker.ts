...
import { IpcExceptionsFilter } from './transports';
...
function setFilter(
  ...
  if (strategy && strategy instanceof ElectronIpcTransport) {
    ...
    const applicationConfig = (app as any)
...
NestFactory.create = async function <
...
>(
  ...
  app.connectMicroservice = function <T extends object = any>(
    ...
    const instance = connectMicroservice.call(
      this,
      options,
      hybridOptions,
...
NestFactory.createMicroservice = async function <T extends object>(
  ...
  const app = (await createMicroservice.call(
    this,
    module,
    options,
  )) as INestMicroservice;
